	...
	print: function(canvasID, data, labels) {
	    ...
	    options: {
	        ...
	        title: function(tooltip, chart) {
						...
						if (datasetIndex === 1) {return [data.Q1, data.MIN];}
						if (datasetIndex === 1) {return [data.Q1, data.MIN];}
						else if (datasetIndex === 2 || datasetIndex === 3) {
							return ["max:"+data.MAX,data.Q3,data.Q2,data.Q1,"min:"+data.MIN];
						}
						else if (datasetIndex === 4) {return [data.MAX, data.Q3];}
	            return '';
	          },
	          label: function(tooltip, chart) {return '';}
	        }
	      },
	      responsive: false,
	      scales: {
	        ...
	        yAxes: [{
				...
				ticks: {max: max + (max * 0.1),min: 0,display: false,},
				stacked: true
	        }]
	      }
	    }
